...
{
    public class MultDivOperation : IObserver
    {
        private const string NumbsRgx = @"(?<!\d)-?\d*[.,]?\d+";
        private const string MultDivRgx = "[\\*\\/]";

        public string Parse(string inExpression)
        {
            var expression = Regex.Match(inExpression, $"({NumbsRgx})\\s?({MultDivRgx})\\s?({NumbsRgx})\\s?");
        ...
        {
            var operation = Regex.Match(expression, $"({NumbsRgx})\\s?({MultDivRgx})\\s?({NumbsRgx})\\s?");
